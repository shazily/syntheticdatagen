...
let dataRequestsData = [];

async function loadDataRequests() {
    try {
        const response = await fetch(`${ADMIN_CONFIG.n8nBaseUrl}/admin/data-requests`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        
        const data = await response.json();
        // N8N returns single object if only 1 result, array if multiple
        dataRequestsData = Array.isArray(data) ? data : (data ? [data] : []);
        displayDataRequests();
    } catch (error) {
        console.error('Error loading data requests:', error);
        const tbody = document.getElementById('data-requests-body');
        if (tbody) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="8" style="text-align: center; padding: 40px; color: #EF4444;">
                        Error loading data requests: ${error.message}
                    </td>
                </tr>
            `;
        }
...
function displayDataRequests() {
    const tbody = document.getElementById('data-requests-body');
    if (!tbody) return;
    
    if (!dataRequestsData || dataRequestsData.length === 0) {
        tbody.innerHTML = `
            <tr>
    ...
    tbody.innerHTML = dataRequestsData.map(request => {
        const statusClass = request.status === 'pending' ? 'status-pending' : 
                           request.status === 'in_progress' ? 'status-progress' :
                           request.status === 'completed' ? 'status-completed' : 'status-rejected';
        ...
        return `
            <tr>
                ...
                <td title="${request.topic || 'N/A'}">${truncateText(request.topic || 'N/A', 30)}</td>
                <td>${request.requested_record_count.toLocaleString()}</td>
                <td>${request.field_count || 'N/A'}</td>
                <td><span class="status-badge ${statusClass}">${request.status}</span></td>
                <td>${createdDate}</td>
                <td>
                    <button class="btn-view" onclick="viewDataRequestDetails(${request.id})" title="View Details">
                        👁️ View
                    </button>
                </td>
            </tr>
...
function viewDataRequestDetails(requestId) {
    const request = dataRequestsData.find(r => r.id === requestId);
    if (!request) return;
    
    const details = `
        <div style="padding: 20px;">
            <h3 style="margin-bottom: 16px; color: var(--text-primary);">Data Request #${request.id}</h3>
            ...
            <div style="display: grid; gap: 12px;">
                ...
                <div class="detail-row">
                    <strong>Topic:</strong> ${request.topic || 'N/A'}
                </div>
                <div class="detail-row">
                    <strong>Requested Record Count:</strong> ${request.requested_record_count.toLocaleString()}
                </div>
                <div class="detail-row">
                    <strong>Field Count:</strong> ${request.field_count || 'N/A'}
                </div>
                <div class="detail-row">
                    <strong>Schema Details:</strong> ${request.schema_details || 'N/A'}
                </div>
    ...
    alert(details); // For now, use alert. Can be replaced with a proper modal later
}

function exportDataRequests() {
    if (!dataRequestsData || dataRequestsData.length === 0) {
    ...
    const csv = [
        ['ID', 'Email', 'Topic', 'Requested Records', 'Field Count', 'Schema Details', 'Status', 'Created At', 'Additional Notes'].join(','),
        ...dataRequestsData.map(r => [
            r.id,
            r.email,
            `"${(r.topic || '').replace(/"/g, '""')}"`,
            r.requested_record_count,
            r.field_count || '',
            `"${(r.schema_details || '').replace(/"/g, '""')}"`,
            r.status,
            new Date(r.created_at).toISOString(),
            `"${(r.additional_notes || '').replace(/"/g, '""')}"`
        ].join(','))
    ].join('\n');
